...
@RestController
//@SwaggerDefinition
@Api(tags = {"Recurrence Type", "Type"})
@RequestMapping("api/v1/type/recurrence")
public class RecurrenceTypeResource implements IPgSoftTypeResource<PgSoftTypeDTO> {
    private final RecurrenceTypeService RecurrenceTypeService;

    public RecurrenceTypeResource(RecurrenceTypeService RecurrenceTypeService) {
        this.RecurrenceTypeService = RecurrenceTypeService;
    }

    @GetMapping(produces = MediaTypes.HAL_JSON_VALUE)
    public PgSoftDTOPagedResources<PgSoftDTOResource<PgSoftTypeDTO>> readAll(@QuerydslPredicate(root = RecurrenceType.class) Predicate predicate, Pageable pageable) {
        return RecurrenceTypeService.readAll(pageable, p -> linkTo(methodOn(this.getClass()).readAll(predicate, p)).withSelfRel()).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> creadByCode(@PathVariable("code") String typeCode) {
        return RecurrenceTypeService.creadByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readById(@PathVariable("id") Long id) {
        return RecurrenceTypeService.readById(id).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readByCode(@PathVariable("code") String typeCode) {
        return RecurrenceTypeService.readByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> update(@PathVariable("id") Long typeId, @RequestBody PgSoftTypeDTO type) {
        return RecurrenceTypeService.update(typeId, type).orElseThrow(NotFoundException::new);
    }
    ...
    public ResponseEntity<Void> deleteByCode(@PathVariable("code") String code) {
        RecurrenceTypeService.deleteByCode(code);
    ...
    public ResponseEntity<Void> deleteById(@PathVariable("id") Long id) {
        RecurrenceTypeService.deleteById(id);
        return new ResponseEntity<>(HttpStatus.ACCEPTED);
    }
}
