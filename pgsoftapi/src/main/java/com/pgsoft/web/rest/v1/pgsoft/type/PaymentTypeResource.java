...
@RestController
//@SwaggerDefinition
@Api(tags = {"Payment Type", "Type"})
@RequestMapping("api/v1/type/payment")
public class PaymentTypeResource implements IPgSoftTypeResource<PgSoftTypeDTO> {
    ...
    public PgSoftDTOPagedResources<PgSoftDTOResource<PgSoftTypeDTO>> readAll(@QuerydslPredicate(root = PaymentType.class) Predicate predicate, Pageable pageable) {
        return PaymentTypeService.readAll(pageable, p -> linkTo(methodOn(this.getClass()).readAll(predicate, p)).withSelfRel()).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> creadByCode(@PathVariable("code") String typeCode) {
        return PaymentTypeService.creadByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readById(@PathVariable("id") Long id) {
        return PaymentTypeService.readById(id).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readByCode(@PathVariable("code") String typeCode) {
        return PaymentTypeService.readByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> update(@PathVariable("id") Long typeId, @RequestBody PgSoftTypeDTO type) {
        return PaymentTypeService.update(typeId, type).orElseThrow(NotFoundException::new);
    }
    ...
    public ResponseEntity<Void> deleteByCode(@PathVariable("code") String code) {
        PaymentTypeService.deleteByCode(code);
