...
//@SwaggerDefinition
@Api(tags = {"City", "Type"})
@RequestMapping("api/v1/type/city")
public class CityResource implements IPgSoftTypeResource<PgSoftTypeDTO> {
    ...
    public PgSoftDTOPagedResources<PgSoftDTOResource<PgSoftTypeDTO>> readAll(@QuerydslPredicate(root = City.class) Predicate predicate, Pageable pageable) {
        return cityService.readAll(pageable, p -> linkTo(methodOn(this.getClass()).readAll(predicate, p)).withSelfRel()).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> creadByCode(@PathVariable("code") String typeCode) {
        return cityService.creadByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readById(@PathVariable("id") Long id) {
        return cityService.readById(id).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readByCode(@PathVariable("code") String typeCode) {
        return cityService.readByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> update(@PathVariable("id") Long typeId, @RequestBody PgSoftTypeDTO type) {
        return cityService.update(typeId, type).orElseThrow(NotFoundException::new);
    }
