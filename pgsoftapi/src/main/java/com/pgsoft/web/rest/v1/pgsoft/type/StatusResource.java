...
@RestController
//@SwaggerDefinition
@Api(tags = {"Status", "Type"})
@RequestMapping("api/v1/type/status")
public class StatusResource implements IPgSoftTypeResource<PgSoftTypeDTO> {
    ...
    public PgSoftDTOPagedResources<PgSoftDTOResource<PgSoftTypeDTO>> readAll(@QuerydslPredicate(root = Status.class) Predicate predicate, Pageable pageable) {
        return StatusService.readAll(pageable, p -> linkTo(methodOn(this.getClass()).readAll(predicate, p)).withSelfRel()).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> creadByCode(@PathVariable("code") String typeCode) {
        return StatusService.creadByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readById(@PathVariable("id") Long id) {
        return StatusService.readById(id).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readByCode(@PathVariable("code") String typeCode) {
        return StatusService.readByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> update(@PathVariable("id") Long typeId, @RequestBody PgSoftTypeDTO type) {
        return StatusService.update(typeId, type).orElseThrow(NotFoundException::new);
    }
    ...
    public ResponseEntity<Void> deleteByCode(@PathVariable("code") String code) {
        StatusService.deleteByCode(code);
