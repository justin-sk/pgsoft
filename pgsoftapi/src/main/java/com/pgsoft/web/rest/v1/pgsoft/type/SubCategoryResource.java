...
@RestController
//@SwaggerDefinition
@Api(tags = {"Sub Category", "Type"})
@RequestMapping("api/v1/type/subcategory")
public class SubCategoryResource implements IPgSoftTypeResource<PgSoftTypeDTO> {
    ...
    public PgSoftDTOPagedResources<PgSoftDTOResource<PgSoftTypeDTO>> readAll(@QuerydslPredicate(root = SubCategory.class) Predicate predicate, Pageable pageable) {
        return SubCategoryService.readAll(pageable, p -> linkTo(methodOn(this.getClass()).readAll(predicate, p)).withSelfRel()).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> creadByCode(@PathVariable("code") String typeCode) {
        return SubCategoryService.creadByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readById(@PathVariable("id") Long id) {
        return SubCategoryService.readById(id).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> readByCode(@PathVariable("code") String typeCode) {
        return SubCategoryService.readByCode(typeCode).orElseThrow(NotFoundException::new);
    }
    ...
    public PgSoftDTOResource<PgSoftTypeDTO> update(@PathVariable("id") Long typeId, @RequestBody PgSoftTypeDTO type) {
        return SubCategoryService.update(typeId, type).orElseThrow(NotFoundException::new);
    }
    ...
    public ResponseEntity<Void> deleteByCode(@PathVariable("code") String code) {
        SubCategoryService.deleteByCode(code);
